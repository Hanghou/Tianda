...
void Integration::initUI()
{
    ...
    // 添加连接管理按钮的信号槽连接
    connect(ui->btnConnectionManager, &QPushButton::clicked, this, [this]() {
        if (m_connectionManagerWidget) {
...
void Integration::initConnections()
{
    ...
    connect(ui->btnEditPowerPresetsStage, &QPushButton::clicked,
            this, &Integration::on_btnEditPowerPresetsStage_clicked);
    connect(ui->btnEditDelayPresets, &QPushButton::clicked,
            this, &Integration::on_btnEditDelayPresets_clicked);
}
...
void Integration::connectLaser(LaserDeviceType type)
{
    ...
    QTimer::singleShot(50, this, [this, driver, config, deviceName, statusIndicator]() {
        ...
        if (success && driver->connect()) {
            ...
            // 连接成功后自动开启激光器
            QTimer::singleShot(100, this, [driver, deviceName]() {
                if (driver->turnOn()) {
                    qDebug() << deviceName << "已自动开启";
                } else {
                    qDebug() << deviceName << "自动开启失败：" << driver->getLastError();
                }
            });
        } else {
...
void Integration::disconnectLaser(LaserDeviceType type)
{
    ...
    if (driver->isConnected()) {
        if (driver->turnOff()) {
            qDebug() << deviceName << "已自动关闭";
        } else {
            qDebug() << deviceName << "自动关闭失败：" << driver->getLastError();
        }
...
void Integration::initPresetTables()
{
    ...
    // 设置列宽
    m_powerPresetTableStage->setColumnWidth(0, 100);
    m_powerPresetTableStage->setColumnWidth(1, 100);
    m_powerPresetTableStage->setColumnWidth(2, 90);
    m_powerPresetTableStage->setColumnWidth(3, 70);
    m_powerPresetTableStage->setColumnWidth(4, 90);
    ...
    // 设置列宽
    m_delayPresetTable->setColumnWidth(0, 120);
    m_delayPresetTable->setColumnWidth(1, 120);
...
void Integration::showPowerPresetEditDialog()
{
    ...
    tableWidget->setHorizontalHeaderLabels({"振镜起始(deg)", "振镜结束(deg)", "种子源泵(mA)", "FOPO泵(A)", "Stokes泵(mA)"});
...
void Integration::showPowerPresetEditDialogStage()
{
    // 创建编辑弹窗
    QDialog *dialog = new QDialog(this);
    dialog->setWindowTitle("编辑电控平台与功率预设");
    dialog->resize(700, 400);
    ...
    QTableWidget *tableWidget = new QTableWidget(dialog);
    tableWidget->setColumnCount(5);
    tableWidget->setHorizontalHeaderLabels({"旋转台角度(deg)", "位移台位置(mm)", "种子源泵(mA)", "FOPO泵(A)", "Stokes泵(mA)"});
    tableWidget->setEditTriggers(QAbstractItemView::AllEditTriggers);
    tableWidget->setAlternatingRowColors(true);
    
    // 从原表格加载数据
    QTableWidget *sourceTable = ui->tableWidgetPowerPresetsStage;
    for (int row = 0; row < sourceTable->rowCount(); ++row) {
        tableWidget->insertRow(row);
        for (int col = 0; col < sourceTable->columnCount(); ++col) {
            QTableWidgetItem *item = sourceTable->item(row, col);
...
void Integration::showDelayPresetEditDialog()
{
    ...
    QTableWidget *tableWidget = new QTableWidget(dialog);
    tableWidget->setColumnCount(2);
    tableWidget->setHorizontalHeaderLabels({"旋转台角度(deg)", "延迟时间(PS)"});
    tableWidget->setEditTriggers(QAbstractItemView::AllEditTriggers);
    tableWidget->setAlternatingRowColors(true);
    
    // 从原表格加载数据
    QTableWidget *sourceTable = ui->tableWidgetDelayPresets;
    for (int row = 0; row < sourceTable->rowCount(); ++row) {
        tableWidget->insertRow(row);
        for (int col = 0; col < sourceTable->columnCount(); ++col) {
            QTableWidgetItem *item = sourceTable->item(row, col);
