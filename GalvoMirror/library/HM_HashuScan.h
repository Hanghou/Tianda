...
HM_HashuAPI  HM_GetDeviceCount(int *nCount);
...
HM_HashuAPI  HM_GetIpAddrByIndex(UINT nDeviceIndex);
HM_HashuAPI  HM_GetConnectStatus(int ipIndex);
HM_HashuAPI  HM_DownloadMarkFile(int ipIndex,char *filePath,HWND hWnd);
HM_HashuAPI  HM_DownloadMarkFileSyn(int ipIndex,char *filePath,HWND hWnd);
...
HM_HashuAPI  HM_SetLaserOn(int ipIndex,bool bEnable);
HM_HashuAPI  HM_SetFrequency(int ipIndex,float frequency,float dutyCycle);
HM_HashuAPI  HM_SetStandbyFrequency(int ipIndex,float standbyFrequency,float standbyDutyCycle);
HM_HashuAPI  HM_GetWorkStatus(int ipIndex);//获取打标状态，1 ready，2 run， 3 Alarm
HM_HashuAPI  HM_SetBoardAlarmStatus(int ipIndex,int status);//设置控制卡Alarm输出信号状态，0默认状态，1报警状态
...
HM_HashuAPI  HM_GetOverangeInfo(int ipIndex);//返回值1，代表有超范围报警
...
//脱机打标
HM_HashuAPI  HM_GetSDCardFlag(int ipIndex);//0代表没有SD卡，1代表有SD卡。有SD卡时把文件下载到SD卡中。无SD卡时下载到控制卡中去，并且文件大小不能超过2M
HM_HashuAPI  HM_SetBurnMode(int ipIndex,int mode);//设置脱机模式，//单文档，多文档标志位，1代表单文档脱机打标，2代表多文档脱机打标（通过二进制IO选择打哪个文档）,3代表多文档按顺序循环触发。
HM_HashuAPI  HM_SetBurnIndex(int ipIndex,int udmIndex);//设置第几个脱机文件标志位
HM_HashuAPI  HM_SetStartBurnFlag(int ipIndex);//开始多文档固话
HM_HashuAPI  HM_GetBurnOverFlag(int ipIndex);//判断是否固话完成
HM_HashuAPI  HM_BurnMarkFile(int ipIndex,bool enable);//脱机时固化打标文件
HM_HashuAPI  HM_GetBurnFileNum(int ipIndex);//获取当前脱机文件个数